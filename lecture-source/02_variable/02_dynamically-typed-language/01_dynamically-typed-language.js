/*
    ...
    변수를 선언할 떄 데이터 타입을 사전에 선언한다. (명시적 타입 선언)
    변수의 타입을 변경할 수 없으머, 변수에 선언한 타입에 맞는 값만 할당할 수 있다.
    컴파일 시점에 타입 체크를 수행하여 타입의 일관성을 강제하며, 런타임 에러를 줄여준다.

    동적 타입(dynamic/week type) 언어 (javascript, python 등)
    데이터 타입을 사전에 선언하지 않는다.
    변수의 선언이 아닌 할당에 의해 타입이 결정 된다.(타입 추론)
...
4. 변수명을 통해 변수의 목적이나 의미를 파악할 수 있도록 함
*/
